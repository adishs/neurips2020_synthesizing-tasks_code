...
def diversity_score_per_trace(idx, trace_info, score_dict, env_type_initial, Z, tree_type, gridsz):
    if( len(Z) == 0 ):
        ...
        # todo : (not needed now?) - unify dissimilarity_score_per_trace and dissimilarity_score_per_trace_2 and individual functions within
...
def cell_level_dissimilarity_2(trace_info, score_dict, env_type_initial, z, tree_type, gridsz):
    # todo optimize with matrix instead of list operations
    ...
    if( env_type_initial == "hoc" ):
        ...
        # todo unify input_task_pregrid and new_task_pregrid to work with '+'
...
def dissimilarity_score_per_trace(idx, trace_info, input_task, score_dict, env_type_initial, tree_type, gridsz):
    # todo : using 0.333 doesn't add up to 1
    if( env_type_initial == "hoc" ):
...
# todo is x,y 0 or 1 index based, then equality sign will change
