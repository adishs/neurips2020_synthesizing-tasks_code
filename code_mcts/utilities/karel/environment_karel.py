...
from utilities.baseclasses.environment_base import border_mask, world_condition, Environment
from utilities.karel.agent_karel import _AgentKarel
from utilities.karel.task_karel import _TaskKarel
...
class EnvironmentKarel(Environment):
    ...
    # todo : change default prob values of coins for random rollout baseline
    def __init__( 
            self, state=None, world_size=(10,10), 
    ...
    def parse_world(self, world_path, parse_mode):
        # bug in Taehoon's code?: changed tuples to match code in karel-for-python repo (https://github.com/alts/karel)
        directions = {
            '>': (1, 0),
            '^': (0, -1),
            '<': (-1, 0),
    ...
    # todo following markers actions not implemented completely 
    # can't pick from empty loc and can't put if agent has no markers not implemented yet

    # todo : make pick_marker code more efficient
    ...
    def pick_marker(self):
        ...
        if( self.mode == "inverse" ):
            if( curr_loc not in self.locked_marker_cells ): 
                ...
                # todo : better to explicitly execute place marker in inp env + pick marker agent logic, although same effect as not doing anything
            else:
    ...
    # todo : make put_marker code more efficient
    @marker_action
    def put_marker(self):
        '''Put marker'''
        curr_loc = str(self.task.agent.position[1]) + '#' + str(self.task.agent.position[0])
        if( self.mode == "inverse" ):
            if( curr_loc not in self.locked_marker_cells ): 
                self.locked_marker_cells.add(curr_loc)
            # todo : should we remove below check by Taehoon of agent holding markers - is this a rule in Karel?
    ...
    # todo : make markers_present code more efficient
            ...
            if( self.mode == "inverse" ):
                ...
                if( curr_loc not in ( self.locked_marker_cells | self.locked_wall_cells ) ):
                    ...
                    if( coin_toss ):
                        ...
                        # add a marker
                        # todo is task.agent.put_marker() the way to add a marker - since it changes the agent state
                        # agent can only add markers it has
                        self.task.markers.append(self.task.agent.position)
                        # todo : max_marker constraint will be enforced since new cell?
                        self.input_marker_cells.add(curr_loc)
                    else:
    ...
    # todo : make no_markers_present code more efficient
    @world_condition
    ...
    # method overriding - post filtering type
